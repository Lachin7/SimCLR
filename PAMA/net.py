...
from PAMA.utils import mean_variance_norm, DEVICE
from PAMA.utils import calc_ss_loss, calc_remd_loss, calc_moment_loss, calc_mse_loss, calc_histogram_loss
from PAMA.utils import calc_tv_loss
from PAMA.hist_loss import RGBuvHistBlock
import torch
...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.hist = RGBuvHistBlock(insz=64, h=150, 
...
class GlobalAlign(nn.Module):
    def __init__(self, channels, reduction_ratio=16, pool_types=['avg']):
...
class Attention(nn.Module):
    def __init__(self, channels, reduction_ratio=2):
        ...
        self.f = nn.Conv2d(channels, channels//reduction_ratio, (1, 1))
        self.g = nn.Conv2d(channels, channels//reduction_ratio, (1, 1))
        self.h = nn.Conv2d(channels, channels//reduction_ratio, (1, 1))

        self.out_conv = nn.Conv2d(channels//reduction_ratio, channels, (1, 1))
        self.relu6 = nn.ReLU6()
        self.softmax = nn.Softmax(dim = -1)
        
    def forward(self, _Fc_, _Fs_, Fs):
        B, C, H, W = _Fc_.shape

        f_Fc = self.relu6(self.f(mean_variance_norm(_Fc_)))
        g_Fs = self.relu6(self.g(mean_variance_norm(_Fs_)))
...
class LocalAlign(nn.Module):
    ...
    def forward(self, F1, F2):
        ...
        return fusion * F1 + (1 - fusion) * F2 
        
class MA(nn.Module):
    def __init__(self, channels):
        super(MA, self).__init__()
        self.conv_in = nn.Conv2d(channels, channels, (3, 3), stride=1)
        self.global_align = GlobalAlign(channels)
        self.attn = Attention(channels)
        self.local_align = LocalAlign(channels)
    ...
    def forward(self, Fc, Fs):
        ...
        Fcs = self.attn(Fc * self.global_align(Fs), Fs * self.global_align(Fc), Fs)
        Fcs = self.relu6(self.conv_out(self.pad(Fcs)))
        Fcs = self.local_align(Fc, Fcs)
