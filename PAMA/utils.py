...
def mu_sigma_loss(A, B):
    mse = torch.nn.MSELoss()
    mean_A, std_A = calc_mean_std(A)
    mean_B, std_B = calc_mean_std(B)
    return mse(mean_A, mean_B) + mse(std_A, std_B)
...
def gram_matrix_loss(A, B):
    mse = torch.nn.MSELoss()
    gram_A = gram_matrix(A)
    gram_B = gram_matrix(B)
    return mse(gram_A, gram_B)
